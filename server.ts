...
async function startServer() {
  ...
  app.post("/api/analyze", async (req, res) => {
    try {
      ...
      // Query Gemini with a fallback strategy if the primary model is busy or overloaded
      const config = {
      ...
      let response;
      try {
        console.log("Attempting scan with 'gemini-3.5-flash'...");
        ...
        console.warn("gemini-3.5-flash model failed or unavailable. Falling back to gemini-3.1-flash-lite. Error details:", geminiError.message || geminiError);
        response = await ai.models.generateContent({
      ...
      res.status(500).json({
        error: `Analysis failed: ${detailedMessage}`,
